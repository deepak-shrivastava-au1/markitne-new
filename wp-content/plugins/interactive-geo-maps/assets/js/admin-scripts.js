...
iMapsBuilder.checkMaxInputVars = function () {
	...
	if (parseInt(numInputs) > parseInt(iMapsOptions.max_input_vars - 200)) {
		warning = document.createElement("div");
		warning.setAttribute("class", "notice notice-error is-dismissible");
		warning.innerHTML = "<p>" + iMapsOptions.messages.maxInputError + "</p>";
...
iMapsBuilder.init = function () {
	...
	iMapsBuilder.prepareMarkerActions();
};

iMapsBuilder.prepareMarkerActions = function () {
	...
	document.addEventListener('mapPointClicked', function (e) {
		...
		var actions = document.getElementById('map_click_events_actions');
		addMarkerButtonIcon.classList.remove('dashicons-yes');
	...
	addMarkerButton.addEventListener('click', function (e) {
		...
		addMarkerButtonIcon.classList.add('dashicons-yes');
		...
		*/
		...
		Object.keys(regionCodes)
			...
			.forEach(function (region) {
				regionData[regionCodes[region].name] = regionCodes[region].id;

				regionDataArray.push({
					label:
						regionCodes[region].name + " " + "(" + regionCodes[region].id + ")",
					value: regionCodes[region].id
				});

				html.appendChild(iMapsBuilder.addRegionButton(regionCodes[region].id, regionCodes[region].name));

				var regionCodeId = document.createElement("code");
				regionCodeId.innerHTML = regionCodes[region].id;
...
iMapsBuilder.updatePreview = function (form, ev) {
	...
	if (ev && ev.target.name == "map_info[projection]" && map) {
		if ("Orthographic" !== ev.target.value) {
...
iMapsBuilder.addRegion = function (regionCode, regionName) {
	var newEntry,
		...
		regionTab,
		addButton = document
			.querySelector('div[data-field-id="[regions]"]')
			.parentNode.querySelector(".csf-cloneable-add");

	regionTab = document.querySelector("[data-section='map_info_3']");
...
iMapsBuilder.addMarker = function (latitude, longitude) {
	var newEntry,
		...
		previewContainer = document.querySelector(".map_wrapper"),
		addButton = document
			.querySelector('div[data-field-id="[roundMarkers]"]')
			.parentNode.querySelector(".csf-cloneable-add");

	markersTab = document.querySelector("[data-section='map_info_4']");
